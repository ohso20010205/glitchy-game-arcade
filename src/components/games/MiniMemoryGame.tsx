...
function getShowSpeed(
  score: number,
  timeLeft: number
): { showMs: number; gapMs: number; label: string } {
...
const MiniMemoryGame = ({ onBack }: { onBack: () => void }) => {
  return (
    <GameWrapper
      ...
      info={{
        description:
          "화면에 표시되는 패턴을 기억한 후 같은 순서로 클릭하세요! 점수가 높아지거나 시간이 얼마 안 남을수록 패턴이 더 빠르게 표시됩니다.",
        bugs: [
        ...
        scoring:
          "정확한 패턴 재현 시 레벨 × 10점. 틀리면 -5점. 점수·잔여시간에 따라 속도 증가!",
      }}
    >
      ...
      if (newInput.length === patternRef.current.length) {
        ...
        if (correct) {
          ...
          setLevel((l) => {
            ...
            timerRef.current = setTimeout(
              () => newRound(nl, Math.max(0, score - 5), timeLeft),
              1200
            );
            return nl;
          });
