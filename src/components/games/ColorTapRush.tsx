...
const ColorTapRush = ({ onBack }: { onBack: () => void }) => {
  return (
    <GameWrapper
      ...
      info={{
        description:
          "화면 상단에 표시된 색상의 타일만 빠르게 탭하세요! 단, 타일에 적힌 이름과 실제 색이 다를 수 있습니다 — 색을 보고 누르세요!",
        bugs: [
  ...
  useEffect(() => {
    ...
    spawnRef.current = setInterval(() => {
      ...
      setTiles((prev) => {
        ...
        if (isGlitched) {
          do {
            labelIdx = Math.floor(Math.random() * TAP_COLORS.length);
    ...
    <div className="game-area w-full h-full absolute inset-0 relative select-none overflow-hidden">
      ...
      {tiles.map((tile) => (
        ...
        >
          ...
          {tile.isGlitched && <span className="text-[7px] mt-0.5 opacity-60">⚠</span>}
        </button>
      ))}
      ...
      {!isRunning && (
        <div className="flex items-center justify-center h-full font-mono text-muted-foreground text-sm text-center px-4">
          Tap tiles matching the target color!
          <br />
