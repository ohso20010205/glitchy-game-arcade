...
export function saveToHallOfFame(gameTitle: string, entry: HallEntry): boolean {
  ...
  try {
    const raw = localStorage.getItem(hofKey(gameTitle));
    if (raw) list = (JSON.parse(raw) as HallEntry[]).filter((e) => e.date === thisMonth);
  } catch { /* noop */ }
...
<<<<<<< HEAD
  ...
  const timerColor =
    timeLeft <= 10 ? "hsl(0 80% 55%)" :
    timeLeft <= 20 ? "hsl(45 100% 55%)" :
    "hsl(180 100% 50%)";
  ...
  return (
    <div className="h-screen flex flex-col bg-background scanline-overlay overflow-hidden">
      ...
      <div className="flex items-center justify-between p-3 border-b border-border bg-muted/50 flex-shrink-0">
        <button onClick={onBack} className="font-pixel text-xs text-foreground hover:text-primary transition-colors">
          ← MENU
        </button>
          ...
          <div className="absolute inset-0 z-10 bg-background/95 overflow-y-auto">
            <div className="flex flex-col items-center py-6 px-4 min-h-full">
              ...
              <div className="w-full max-w-md space-y-3 mb-5">
                ...
                <div className="text-center font-mono text-xs text-muted-foreground">제한 시간: {duration}초</div>
              ...
              {editingNick ? (
                <div className="w-full max-w-xs space-y-2 mb-4">
                  <p className="font-pixel text-xs text-accent text-center">닉네임 설정 (명예의 전당용)</p>
                  <div className="flex gap-2">
                    <input
                      ...
                      maxLength={12}
                    />
                    <button onClick={handleSetNickname} className="font-pixel text-xs px-3 py-2 bg-primary text-primary-foreground rounded">
                      OK
                    </button>
                  </div>
                </div>
        ...
        {gameOver && (
          <div className="absolute inset-0 z-10 bg-background/92 overflow-y-auto">
            <div className="flex flex-col items-center py-8 px-4 min-h-full">
              <h3 className="font-pixel text-lg text-destructive glitch-text mb-1">GAME OVER</h3>
              ...
              {hofList.length > 0 && (
                <div className="w-full max-w-xs mb-5">
                  <p className="font-pixel text-[10px] text-yellow-400 text-center mb-2">🏆 {title} 명예의 전당</p>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            border: `1px solid ${isMe ? "hsl(120 100% 50% / 0.5)" : "hsl(var(--border))"}`,
                          }}
                        >
                          <span className="text-base">{MEDAL[i]}</span>
                          <span className="font-mono text-xs flex-1 mx-2 truncate" style={{ color: isMe ? "hsl(120 100% 60%)" : undefined }}>
                            {e.nickname}
                          </span>
              ...
              <div className="flex gap-3">
                <button onClick={startGame} className="font-pixel text-xs px-5 py-2 bg-primary text-primary-foreground rounded">
                  RETRY
                </button>
                <button onClick={onBack} className="font-pixel text-xs px-5 py-2 bg-muted text-foreground rounded border border-border">
                  MENU
                </button>
              </div>
=======
...
function HofPanel({ title }: { title: string }) {
  ...
  if (list.length === 0) return (
    <div className="bg-muted/40 border border-yellow-400/20 rounded-lg p-3 text-center">
      ...
      <p className="font-mono text-xs text-muted-foreground">아직 기록이 없습니다. 첫 번째 주인공이 되세요!</p>
    </div>
  );
  ...
  return (
    <div className="bg-muted/40 border border-yellow-400/20 rounded-lg p-3">
      ...
      <div className="space-y-1">
        {list.map((e, i) => (
          <div key={i} className="flex items-center gap-2 px-2 py-1 rounded" style={{ background: "hsl(240 10% 10%)" }}>
            <span>{MEDAL[i]}</span>
            <span className="font-mono text-xs flex-1 truncate text-foreground/80">{e.nickname}</span>
            <span className="font-pixel text-xs text-secondary">{e.score}</span>
          </div>
        ))}
      </div>
      <p className="font-mono text-[9px] text-muted-foreground/40 text-center mt-1.5">이번 달 상위 3명 · 매월 초기화</p>
    </div>
  );
}
